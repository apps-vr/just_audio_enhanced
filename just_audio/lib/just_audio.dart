...
class AudioPlayer {
  ...
  AudioPlayer({
    ...
    _androidAudioSessionIdSubject.addStream(playbackEventStream.map((event) => event.androidAudioSessionId).distinct().handleError((Object err, StackTrace stackTrace) {/* noop */}));
    ...
    _playerStateSubject.addStream(Rx.combineLatest2<bool, PlaybackEvent, PlayerState>(playingStream, playbackEventStream, (playing, event) => PlayerState(playing, event.processingState))
        .distinct()
  ...
  Future<Duration?> setUrl(
    ...
    bool preload = true,
      ...
      setAudioSource(AudioSource.uri(Uri.parse(url), headers: headers), initialPosition: initialPosition, preload: preload);
  ...
  Future<Duration?> setFilePath(
    ...
    bool preload = true,
      ...
      setAudioSource(AudioSource.file(filePath), initialPosition: initialPosition, preload: preload);
  ...
  Future<void> setShuffleModeEnabled(bool enabled) async {
    ...
    await (await _platform).setShuffleMode(SetShuffleModeRequest(shuffleMode: enabled ? ShuffleModeMessage.all : ShuffleModeMessage.none));
  }
  ...
  Future<void> setCanUseNetworkResourcesForLiveStreamingWhilePaused(final bool canUseNetworkResourcesForLiveStreamingWhilePaused) async {
    ...
    await (await _platform)
        .setCanUseNetworkResourcesForLiveStreamingWhilePaused(SetCanUseNetworkResourcesForLiveStreamingWhilePausedRequest(enabled: canUseNetworkResourcesForLiveStreamingWhilePaused));
  }
    ...
    switch (processingState) {
      ...
      default:
        try {
          ...
          _playbackEventSubject.add(_playbackEvent);
          _positionDiscontinuitySubject.add(PositionDiscontinuity(PositionDiscontinuityReason.seek, prevPlaybackEvent, _playbackEvent));
  ...
  Future<void> _internalSetAndroidAudioAttributes(AudioPlayerPlatform platform, AndroidAudioAttributes audioAttributes) async {
    if (!_isAndroid() && !_isUnitTest()) return;
    await platform.setAndroidAudioAttributes(SetAndroidAudioAttributesRequest(contentType: audioAttributes.contentType.index, flags: audioAttributes.flags.value, usage: audioAttributes.usage.value));
  }
  ...
  Future<Duration?>? _setPlatformActive(bool active, {Completer<void>? playCompleter, bool force = false}) {
    ...
    Future<AudioPlayerPlatform> setPlatform() async {
      ...
      final platform = active
          ...
          : (_idlePlatform = _IdleAudioPlayer(id: id, sequenceStream: sequenceStream));
      ...
      if (active) {
        ...
        if (checkInterruption()) return platform;
        await platform.setShuffleMode(SetShuffleModeRequest(shuffleMode: shuffleModeEnabled ? ShuffleModeMessage.all : ShuffleModeMessage.none));
        if (checkInterruption()) return platform;
  ...
  Future<void> _disposePlatform(AudioPlayerPlatform platform) async {
    if (platform is _IdleAudioPlayer) {
      ...
      _nativePlatform = null;
      try {
        await _pluginPlatform.disposePlayer(DisposePlayerRequest(id: id));
      } catch (e) {
...
class PlaybackEvent {
  ...
  @override
  String toString() => "{processingState=$processingState, updateTime=$updateTime, updatePosition=$updatePosition, bufferedPosition=$bufferedPosition, duration=$duration, currentIndex=$currentIndex}";
}
...
class ConcatenatingAudioSource extends AudioSource {
  ...
  Future<void> add(AudioSource audioSource) async {
    ...
    if (_player != null) {
      ...
      await (await _player!._platform).concatenatingInsertAll(ConcatenatingInsertAllRequest(id: _id, index: index, children: [audioSource._toMessage()], shuffleOrder: List.of(_shuffleOrder.indices)));
    }
  }
  ...
  Future<void> addAll(List<AudioSource> children) async {
    ...
    if (_player != null) {
      ...
      await (await _player!._platform)
          .concatenatingInsertAll(ConcatenatingInsertAllRequest(id: _id, index: index, children: children.map((child) => child._toMessage()).toList(), shuffleOrder: List.of(_shuffleOrder.indices)));
    }
  }
  ...
  Future<void> removeAt(int index) async {
    ...
    if (_player != null) {
      _player!._broadcastSequence();
      await (await _player!._platform).concatenatingRemoveRange(ConcatenatingRemoveRangeRequest(id: _id, startIndex: index, endIndex: index + 1, shuffleOrder: List.of(_shuffleOrder.indices)));
    }
  }
  ...
  Future<void> removeRange(int start, int end) async {
    ...
    if (_player != null) {
      _player!._broadcastSequence();
      await (await _player!._platform).concatenatingRemoveRange(ConcatenatingRemoveRangeRequest(id: _id, startIndex: start, endIndex: end, shuffleOrder: List.of(_shuffleOrder.indices)));
    }
  }
  ...
  Future<void> move(int currentIndex, int newIndex) async {
    ...
    if (_player != null) {
      _player!._broadcastSequence();
      await (await _player!._platform).concatenatingMove(ConcatenatingMoveRequest(id: _id, currentIndex: currentIndex, newIndex: newIndex, shuffleOrder: List.of(_shuffleOrder.indices)));
    }
  }
  ...
  Future<void> clear() async {
    ...
    if (_player != null) {
      _player!._broadcastSequence();
      await (await _player!._platform).concatenatingRemoveRange(ConcatenatingRemoveRangeRequest(id: _id, startIndex: 0, endIndex: end, shuffleOrder: List.of(_shuffleOrder.indices)));
    }
  }
  ...
  @override
  AudioSourceMessage _toMessage() =>
      ConcatenatingAudioSourceMessage(id: _id, children: children.map((child) => child._toMessage()).toList(), useLazyPreparation: useLazyPreparation, shuffleOrder: _shuffleOrder.indices);
}
...
class ClippingAudioSource extends IndexedAudioSource {
  ...
  @override
  AudioSourceMessage _toMessage() => ClippingAudioSourceMessage(id: _id, child: child._toMessage() as UriAudioSourceMessage, start: start, end: end, tag: tag);
}
...
class LockCachingAudioSource extends StreamAudioSource {
  ...
  Future<HttpClientResponse> _fetch() async {
    ...
    subscription = response.listen((data) async {
      ...
      sink.add(data);
      final readyRequests = _requests.where((request) => !originSupportsRangeRequests || request.start == null || (request.start!) < _progress).toList();
      final notReadyRequests = _requests.where((request) => originSupportsRangeRequests && request.start != null && (request.start!) >= _progress).toList();
...
_ProxyHandler _proxyHandlerForSource(StreamAudioSource source) {
  Future<void> handler(_ProxyHttpServer server, HttpRequest request) async {
    ...
    if (rangeRequest != null && sourceResponse.offset != null) {
      final range = _HttpRangeResponse(sourceResponse.offset!, sourceResponse.offset! + sourceResponse.contentLength! - 1, sourceResponse.sourceLength);
  ...
  Future<void> handler(_ProxyHttpServer server, HttpRequest request) async {
    ...
    try {
      ...
      if (headers != null && request.uri.path.toLowerCase().endsWith('.m3u8') ||
          ['application/x-mpegURL', 'application/vnd.apple.mpegurl'].contains(request.headers.value(HttpHeaders.contentTypeHeader))) {
...
class _IdleAudioPlayer extends AudioPlayerPlatform {
  ...
  Duration? _getDurationAtIndex(int? index) => index != null && _sequence != null && index < _sequence!.length ? _sequence![index].duration : null;
  ...
  @override
  Future<SetCanUseNetworkResourcesForLiveStreamingWhilePausedResponse> setCanUseNetworkResourcesForLiveStreamingWhilePaused(SetCanUseNetworkResourcesForLiveStreamingWhilePausedRequest request) async {
